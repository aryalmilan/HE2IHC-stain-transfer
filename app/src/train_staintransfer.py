...
from stain_transfer import StainTransfer, VAE_encode, VAE_decode, initialize_unet, initialize_vae
...
def main(args):
    ...
    params_gen = StainTransfer.get_traininable_params(unet, vae_a2b, vae_b2a)
    ...
    dataset_train = UnpairedDataset(dataset_folder=args.dataset_folder, image_prep=args.train_img_prep,stain=args.stain, split="train", tokenizer=tokenizer)
    ...
    print(f"Total data:{len(train_dataloader)}")
    ...
    l_images_tgt_test = []
    for ext in ["*.jpg", "*.jpeg", "*.png", "*.bmp"]:
        l_images_tgt_test.extend(glob(os.path.join(args.dataset_folder, f"test_{args.stain}", ext)))
    ...
    for epoch in range(first_epoch, args.max_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                """
                # A -> fake B -> rec A
                cyc_fake_b = StainTransfer.forward_with_networks(img_a, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_a2b_emb)
                cyc_rec_a = StainTransfer.forward_with_networks(cyc_fake_b, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_b2a_emb)
                ...
                # B -> fake A -> rec B
                cyc_fake_a = StainTransfer.forward_with_networks(img_b, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_b2a_emb)
                cyc_rec_b = StainTransfer.forward_with_networks(cyc_fake_a, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_a2b_emb)
                ...
                """
                fake_a = StainTransfer.forward_with_networks(img_b, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_b2a_emb)
                fake_b = StainTransfer.forward_with_networks(img_a, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_a2b_emb)
                ...
                Identity Objective
                """
                idt_a = StainTransfer.forward_with_networks(img_b, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_a2b_emb)
                loss_idt_a = crit_idt(idt_a, img_b) * args.lambda_idt
                loss_idt_a += net_lpips(idt_a, img_b).mean() * args.lambda_idt_lpips
                idt_b = StainTransfer.forward_with_networks(img_a, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_b2a_emb)
                    ...
                    if global_step % args.validation_steps == 1:
                        ...
                        for idx, input_img_path in enumerate(tqdm(l_images_src_test)):
                            if idx > args.validation_num_images and args.validation_num_images > 0:
                                break
                            outf = os.path.join(fid_output_dir, input_img_path.split('/')[-1])#f"{idx}.png")
                            with torch.no_grad():
                                ...
                                eval_fake_b = StainTransfer.forward_with_networks(img_a, "a2b", eval_vae_enc, eval_unet,
